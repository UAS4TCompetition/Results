% this code is used to identify the trajectories that pass by the green
% polygon in the correct direction 
% 
...
for k=1:length(TR)
    ...
    for p=1:3 % loop over the three polygons
        ...
        if sum(in)>2
            ...
            if arclen3>arclen1 &arclen4<arclen2
                TR{k,8}=p; % identify polygon that trajectory pass by based on 1- headng of the trajectory 2- the trajectory intersect with the polygon
            end
        end
    end
end
TR=TR(find(cell2mat(TR(:,8))==3),:); % select the trajectories that belong to polygon number 3 (the green polygon) 
RP=kml2struct('Reference Point.kml');
ply=kml2struct('Green polygon lanes.kml');% reading the kml file to get the three polygons
plye=kml2struct('Green polygon lanes - Extended.kml');% reading the kml file to get the three polygons

% transforming to plan coordinates---------
    flatearth_points=[];
   for p=1:5
       flatearth_points = [flatearth_points;lla2flat([ply(5+p).Lat,ply(5+p).Lon,0],[RP.Lat(1),RP.Lon(1)],0,0)]; 
   end
%% caculating the distance from the reference points
for k=1:size(TR,1)
   ...
   %% imputing points within the trajectory which are missed because of inaccuries   
      st=find(TR{k,7}.distance~=0,1);
...
%         x=TR{k,7}.time(st+idx-1);
   ...
   TR{k,7}.distance(st+idxx-1)=yi;
    ...
    writetable(TR{k,7},strcat('C:\Users\elhenawy\OneDrive - Queensland University of Technology\compition\green_poly\T',num2str(k),'.csv'));
end
