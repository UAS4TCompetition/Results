...
%     end
% end
% TR=TR(find(cell2mat(TR(:,8))==1),:);
% RP=kml2struct('Reference Point.kml');
% ply=kml2struct('red polygon lanes.kml');% reading the kml file to get the three polygons
% plye=kml2struct('red polygon lanes - Extended.kml');% reading the kml file to get the three polygons
% 
...
%         x=TR{k,7}.time(st+idx-1);
   ...
   TR{k,7}.distance(st+idxx-1)=yi;
    ...
    writetable(TR{k,7},strcat('C:\Users\elhenawy\OneDrive - Queensland University of Technology\compition\red_poly\T',num2str(k),'.csv'));
end
